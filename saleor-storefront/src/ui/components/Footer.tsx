...
export async function Footer({ channel }: { channel: string }) {
	...
	return (
		<footer className="bg-[#232F3E] text-white shadow-inner">
			<div className="mx-auto max-w-screen-2xl px-4 lg:px-8">
					...
					{footerLinks.menu?.items?.map((item) => {
						return (
							<div key={item.id}>
								...
								<ul className="mt-4 space-y-4 [&>li]:text-gray-300">
				...
				<div className="flex flex-col justify-between border-t border-white/10 py-10 sm:flex-row">
					<p className="text-sm">Copyright &copy; {currentYear} allistore.</p>
				</div>
			</div>
		</footer>
	);
